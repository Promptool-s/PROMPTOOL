  ...
  <div className={`absolute inset-0 flex flex-col transition-opacity duration-700 ${visible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
    ...
    <div className="mt-3 space-y-2 px-1 select-none" onCopy={e => e.preventDefault()}>
      <div className="flex items-center gap-2.5">
        {item.avatar_url
          ? <img src={proxyImg(item.avatar_url)} alt="" className="h-7 w-7 rounded-full object-cover ring-1 ring-white/10 pointer-events-none" draggable={false} />
          : <div className="flex h-7 w-7 items-center justify-center rounded-full bg-cyan-500/30 text-xs font-bold text-cyan-400">{(item.username || '?')[0].toUpperCase()}</div>
        }
      ...
      <p className="line-clamp-2 text-xs leading-5 text-slate-400 italic">"{item.prompt_usuario}"</p>
    </div>
  </div>
)
  ...
  <div className="flex items-center justify-center gap-1.5 pt-1">
    {Array.from({ length: total }).map((_, i) => (
      <button key={i} onClick={() => onSelect(i)} aria-label={`Slide ${i + 1}`} aria-current={i === current}
        className={`rounded-full transition-all duration-300 focus-visible:outline focus-visible:outline-2 focus-visible:outline-cyan-400 focus-visible:outline-offset-2 ${i === current ? 'h-1.5 w-5 bg-cyan-400' : 'h-1.5 w-1.5 bg-slate-600 hover:bg-slate-500'}`} />
    ))}
  </div>
)
...
const CommunitySlideshow = ({ lang }) => {
  ...
  if (!slides.length) return <div className="flex h-full items-center justify-center text-sm text-slate-400">{c.noSlides}</div>

  return (
    <div className="flex h-full flex-col" role="region" aria-roledescription="carousel" aria-label={c.community}>
      <p className="mb-3 text-[10px] font-semibold uppercase tracking-widest text-slate-400">{c.community}</p>
