...
const Toggle = ({ checked, onChange, id, disabled = false }) => (
  <button
    ...
    className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-slate-500 disabled:opacity-40 disabled:cursor-not-allowed ${
  ...
  >
    <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full shadow ring-0 transition duration-200 ${
      checked ? 'translate-x-4 bg-white dark:bg-slate-900' : 'translate-x-0 bg-white dark:bg-slate-400'
    }`} />
  </button>
)
