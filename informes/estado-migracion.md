...
1. **`App.jsx`** — **parcial (2026-07-21)**: feed de imágenes, `join_company_by_link` y `user_onboarded` ya migrados a `/api/imagenes` / `/api/enterprise/unirse` / `PUT /api/usuarios/me`; el reveal manual también, pero solo para usuarios logueados (gating server-side real). **Sigue sin migrar el núcleo**: el submit completo (scoring vía Groq client-side, ELO calculado y escrito por el cliente, insert directo a `intentos`) — es la parte más delicada, decidido con el usuario dejarla para una sesión aparte y más despacio (no se puede probar en browser desde acá). Ver `todo-migracion-frontend.md` para el detalle línea por línea y la nota de producto sobre guests (van a perder el "carry-over" de intentos pre-registro).
