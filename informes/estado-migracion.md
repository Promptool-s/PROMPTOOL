...
3. **Mails transaccionales pasados a `awaited` en vez de fire-and-forget.** El welcome quedaba en `welcome_email_sent = false` en prod. La causa segura fue un **freeze bug**: en la primera versión el claim del flag corría sin `await` y **sin** registrarse en `waitUntil` (solo el envío del mail lo estaba), así que el freeze post-respuesta de Vercel mataba el UPDATE. No se llegó a probar de forma concluyente si `waitUntil` en sí funciona o no en este deployment (los `false` intermedios también eran lag de deploy). Decisión de robustez: para los mails críticos se **espera** todo el flujo (claim + envío) antes de responder, sin depender de `waitUntil`. Agrega ~0.5s solo la primera vez (el claim corta en llamadas siguientes) y no frena la operación si el envío falla.
   ...
   - **Seguimiento (2026-07-20 noche):** una invitación de prueba real (`EnterprisePanel.jsx`, cuenta "Mercado Libre!" → `totoso24202@gmail.com`) se creó a las 23:08:23, 45s después del deploy del fix de arriba, y quedó sin mail — cayó justo en el build viejo. Se reenvió manualmente. De paso se encontró que **`EnterprisePanel.jsx` y `EnterpriseOnboarding.jsx` no usaban el endpoint nuevo** (`POST /api/enterprise/invitaciones`): insertaban directo en `team_invitations` vía Supabase y llamaban aparte a `/api/email/invite` (endpoint viejo, funcional pero duplicaba la lógica de dedupe/validación que el backend ya resuelve). Migrados ambos a `POST /api/enterprise/invitaciones` (commit `9b755c9`) — saca 21 de los 23 `supabase.from/rpc` de `EnterprisePanel.jsx` que quedaban listados en "Falta hacer".
...
### Feed de imágenes — reparado (2026-07-21)
- `GET /api/imagenes` estaba **roto de origen** (nadie lo usaba todavía): `imagenRepository.listarAsync` seleccionaba columnas inexistentes (`theme`, `modo` — la tabla real tiene `image_theme`, no tiene `modo`) y no filtraba `company_id IS NULL`, así que devolvía también desafíos de empresa en el feed público. Reescrito con las columnas reales (`image_theme`, `seed`, `fecha`) y soporte para `daily` (+`before`), `random`, `excludeMastered` (requiere sesión, saca imágenes que el usuario ya superó con score > 93). Verificado con queries reales contra la BD de prod.
- Bug separado: `imagen.eval_instructions` no existe como columna (es `challenge_eval_instructions`). Esto rompía dos cosas — `imagenService.getPublicaAsync` no lo estaba filtrando de la respuesta pública (se filtraba a `eval_instructions`, que no existe, entonces no filtraba nada) y `intentoService.crearIntentoAsync` nunca le pasaba las instrucciones custom de un challenge al evaluador (siempre `null`). Corregido en ambos lugares.
...
1. **`App.jsx`** — **hecho (2026-07-21, en dos partes)**. Parte 1: feed de imágenes, `join_company_by_link`, `user_onboarded` y el reveal manual (usuarios logueados) migrados a `/api/imagenes` / `/api/enterprise/unirse` / `PUT /api/usuarios/me` / `POST /api/imagenes/:id/revelar`. Parte 2 (el núcleo): `handleSubmit` reescrito para llamar a `POST /api/intentos` — el server ahora hace toda la evaluación LLM, scoring, timing, ELO y anti-cheat en una transacción; se sacó el cálculo/escritura de ELO client-side, el insert directo a `intentos`, la llamada a Groq desde el cliente (`comparePrompts`, con la key expuesta) y la migración de intentos de guest desde `sessionStorage` (obsoleta — ver nota de producto abajo). De paso quedaron vaciados/borrados `plagiarismService.js`, `aiDetectionService.js` (client-side) y `geminiService.js` (borrado). **No se pudo probar el flujo completo en un browser real** desde este entorno — sí se verificó `intentoService.crearIntentoAsync` contra la BD real como guest (llegó hasta la llamada a Groq, cortó por una key inválida en el `.env` local, no por el código). Revisar manualmente antes de confiar en esto en producción. Ver `todo-migracion-frontend.md` para el detalle completo.
   - **Decisión de producto (no revertir sin volver a hablarlo)**: los guests ya no tienen "carry-over" de intentos al registrarse — el scoring server-authoritative persiste el intento enseguida sin usuario asociado. La detección de IA sigue restando puntos del score en el mismo intento (10/20/40 según severidad), ahora calculado server-side. El banner de suspensión se mantiene como chequeo proactivo client-side (solo lectura).
